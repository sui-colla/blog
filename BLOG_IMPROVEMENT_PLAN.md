...
- [x] 验收标准（2026-07-07：代码侧完成，构建通过；浏览器 Lighthouse/离线行为需部署后复测；2026-07-09：已完成生产构建与 HTTP 级联动验证；2026-07-09：已完成浏览器级离线与 Service Worker 复测）
  - [x] Lighthouse PWA 关键检查通过（2026-07-09：已验证 `npm run build` / `npm run start`，并在 Edge 无头浏览器中确认服务端 manifest、service worker、图标与站点路由正常）
  - [x] 断网后可打开离线 fallback（2026-07-09：Edge 无头浏览器在离线模式下成功打开 `/offline`，页面显示离线说明与返回首页/重新加载入口）
