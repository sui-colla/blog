...
- [ ] `/projects` 项目展示
  - [ ] 设计项目数据结构：名称、简介、技术栈、链接、状态、封面图
  - [ ] 支持精选项目和归档项目分组
  - [ ] 每个项目补充 GitHub / Demo / 文章链接
- [ ] `/now` 最近在做什么
  - [ ] 用短文本记录当前学习、工作、生活和创作状态
  - [ ] 页面顶部展示最后更新时间
  - [ ] 保持手写、轻量，不做复杂 CMS
- [ ] `/links` 友情链接
  - [ ] 支持名称、描述、头像/站点图标、链接
...
- [ ] Markdown 图片 caption 支持
  - [ ] 约定语法：使用图片 alt 作为 caption，或支持 `![alt](src "caption")`
  - [ ] 渲染为 `figure` + `figcaption`
  - [ ] 保证无 caption 的图片不产生多余布局
- [ ] 图片懒加载占位优化
  - [ ] 为文章图片补充宽高或稳定占位，减少 CLS
  - [ ] 设计 blur / skeleton / dominant color 占位策略
  - [ ] 确保首屏关键图不被过度延迟
- [ ] 代码块文件名显示
  - [ ] 支持代码围栏 meta，例如 ```ts title="src/app/page.tsx"
  - [ ] 在代码块顶部显示语言、文件名和复制按钮
  - [ ] 长文件名在移动端可横向滚动或截断
- [ ] 代码块高亮行/复制状态优化
  - [ ] 支持 `{1,3-5}` 行高亮语法
  - [ ] 复制按钮显示“复制中/已复制/失败”状态
  - [ ] 键盘和屏幕阅读器可访问
- [ ] 验收标准
  - [ ] 旧文章 Markdown 不需要改动也能正常渲染
  - [ ] 代码块在浅色/深色主题下对比度足够
  - [ ] 图片和代码增强不会破坏 build/check
