            ...
            <section key={category.title}>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {category.items.map((item) => (
                  <article key={item.name} className="rounded-2xl border border-orange-100 bg-white/80 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70">
                    ...
                    <div className="mt-4 flex flex-wrap gap-3 text-sm font-medium">
                      {item.url && (
                        <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:text-orange-700 dark:text-orange-400">
                          {t("uses.visit")} ↗
                        </a>
                      )}
                      {item.postUrl && (
                        <Link href={item.postUrl} className="text-orange-600 hover:text-orange-700 dark:text-orange-400">
                          {t("uses.readPost")} →
                        </Link>
                      )}
                    </div>
